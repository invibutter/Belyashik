...
- Style Dictionary v5 — generates two files from `tokens/tokens.json` (W3C DTCG format: `$value`/`$type`) via `npm run tokens:build`:
  - `src/styles/tokens.css` — full token set as plain `:root` CSS custom properties
  - `src/styles/theme.css` — `color`/`spacing`/`radius` tokens only, wrapped in an `@theme { ... }` block (Tailwind v4's theme mechanism) via a custom `css/theme` format registered in `style-dictionary.config.js`; imported into `index.css` right after `@import "tailwindcss";` so token names become real Tailwind utilities (e.g. a `color.brand.blue` token makes `bg-brand-blue` a valid class)
  - `tokens/tokens.json` may contain semantic alias tokens (e.g. `color.brand.*`) that reference a primitive token via `{color.path.to.token}` DTCG reference syntax — these aren't necessarily 1:1 with a Figma variable name; check the alias target before assuming it maps directly to Figma
